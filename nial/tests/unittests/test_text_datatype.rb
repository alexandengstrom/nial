...
class Test_Text < Test::Unit::TestCase
  ...
  def test_right_justify
    ...
    x = Text.new("test")
    z = x.right_justify(Text.new([1,2,3]), scope, position)
    assert_equal(ConversionError, z.class)
  end
  ...
  def test_sub
    ...
    # Using regex (in the tests we need to double escpae \d but not necessary in the language)
    x = Text.new("a string with 233 inside")
    pattern = Text.new("\\d+")
    new_value = Text.new("numbers")
    z = x.sub(pattern, new_value, scope, position)
    assert_equal(Text, z.class)
    assert_equal("a string with numbers inside", z.value)
